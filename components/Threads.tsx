...
export default function Threads({ documentId, data, formType, viewer, setModal, sessionKey, style }) {
  ...
  return (
    <div className={styles.root} style={style}>
      {threads?.map((each, index) => {
        ...
        return (
          <ThreadItem isLast={index === data.length - 1} key={each.id} sessionKey={sessionKey} setModal={setModal} threadId={each.id} viewer={viewer} documentId={documentId}>
            <div className={styles.byline}>
              {author} ⎯ {Utilities.timeAgo(each.created_at)}
            </div>
            <div className={styles.body}>{each.data.plainText}</div>
