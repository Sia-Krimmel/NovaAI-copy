...
export default function CardWithAddedItems({ disabled, canEdit, minerIds, setMinerIds }: CardWithAddedItems) {
        ...
        {minerIds.map(({ miner_id, ip_address }, index) => (
          <div key={index} className={gridStyles.twoColumnGridTiny} style={{ gridColumn: 'span 3', paddingRight: '16px' }}>
            ...
            <div className={styles.cardItemOptions} style={{ paddingBottom: '16px', gridColumn: '4' }}>
              ...
              {canEdit && (
                <Button style={ButtonStyleEnum.CIRCLE_BORDER_BLACK} onClick={() => console.log('Editing item at index:', index)} disabled={disabled}>
                  <EditSVG />
                </Button>
              )}
            </div>
